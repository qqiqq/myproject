...
class Retriever(object):
    ...
    def get_file(self,url,default='index.html'):
        ...
        filepath = '%s%s' %s (host,parsed.path)
    ...
    def parse_links(self):
        ...
        parser = HTMLParser(formatter.AbstractFormatter(
            formatter.DumbWriter(cStrngIO.StringIO)))
        parser.feed(data)
        parser.close()
...
class Crawler(object):
    ...
    def get_page(self,url,media=False):
        'Download page & parse links, add to queue if need'
        r = Retriever(url)
        fname = r.download()[0]
        if fname[0] == '*':
...
def main():
    if len(sys.argv) > 1:
        ...
        try:
            url = raw_input('Please enter starting URL: ')
        except (KeyboardInterrupt,EOFError) as e:
            url = ''
    if not url:
        return
    if not url.startwith('http://') and not url.startwith('ftp://'):
        url = 'http://%s/' % url
    robot = Crawler(url)
    robot.go()
